---
layout: post
title: AI 沙箱 Workspace 存算分离：挂载型方案讨论
---

给 Agent 配沙箱这件事，现在几乎成了 AI 工具的标配：代码改在隔离环境里，跑完再看结果。真正麻烦的不是「怎么开一个沙箱」，而是 workspace 往哪放——文件要跟着会话活下去，计算节点却随时可以扔。开放注册的量又估不准，存储还得能扩、能换。下面聊一套挂载型的存算分离想法。
